...
const StudentProfile = mongoos.model("StudentProfile");
...
module.exports.studentsGetAll = function(req, res){
    ...
    if(searchString){
        ...
        StudentProfile.find().select("-attendance").skip(offset).limit(count).exec(function(err, students){
            if(err){
                response.status = 500;
                response.message = err;
            }else {
...
module.exports.studentsGetOne = function(req, res){
    ...
    StudentProfile.findById(id).select("-attendance").exec(function(err, student){
        let response = {
            status : 201,
            message: student
        }
...
module.exports.studentsDeleteOne = function(req, res){
    ...
    StudentProfile.findByIdAndRemove(id).exec(function(err, deletedStudent){
        let response = {
            status: 204,
